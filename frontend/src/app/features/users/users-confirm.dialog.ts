...
export interface UsersConfirmData {
  title: string;
  message: string;
  confirmLabel?: string;
  icon?: string;
  ...
  template: `
    <div class="p-1">
      ...
      <mat-dialog-actions align="end" class="!gap-2 !px-6 !pb-5">
        ...
        <button mat-flat-button class="gcc-btn-primary !rounded-xl" type="button" (click)="dialogRef.close(true)">
...
export class UsersConfirmDialog {
  readonly data = inject<UsersConfirmData>(MAT_DIALOG_DATA);
  readonly dialogRef = inject(MatDialogRef<UsersConfirmDialog, boolean>);
}

...
@Component({
  selector: 'app-skill-family-page',
  imports: [FormsModule, GccPageHeader, GccSelect, GccEmptyState, GccStatusTag, MatTableModule, MatButtonModule, MatIconModule],
  template: `
    <gcc-page-header
      title="Familles de compétences"
      subtitle="Chaque famille appartient à un domaine et accueille les compétences unitaires."
      icon="account_tree"
      [crumbs]="crumbs"
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-1">
          <div class="sticky top-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="mb-4 flex items-start justify-between gap-3">
              <div>
                <h2 class="text-sm font-semibold text-navy">{{ editingId() ? 'Modifier la famille' : 'Nouvelle famille' }}</h2>
                <p class="mt-1 text-xs text-slate-500">Code numérique proposé à la création, rattaché à un domaine.</p>
              </div>
              @if (editingId()) {
            ...
            <div class="flex flex-col gap-4">
              <label class="block">
                <span class="mb-1 block text-sm font-medium text-slate-600">Domaine</span>
                <gcc-select [options]="domainOptions()" [value]="domainId()" (valueChange)="onDomainChange($event)" placeholder="Choisir un domaine" />
              </label>
              <label class="block">
              ...
              <label class="block">
                ...
                <input class="gcc-input" placeholder="Ex. Comptabilité" [(ngModel)]="name" />
              </label>
              <label class="block">
              ...
              <label class="block">
                ...
                <textarea class="gcc-input min-h-20" rows="2" placeholder="Compétences regroupées dans cette famille" [(ngModel)]="description"></textarea>
              </label>
            </div>
      ...
      <div [class]="canManage() ? 'lg:col-span-2' : 'lg:col-span-3'">
        @if (!rows().length) {
          <gcc-empty-state title="Aucune famille" message="Créez une famille rattachée à un domaine." />
        } @else {
          <div class="gcc-table shadow-sm">
            <div class="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-5 py-3.5">
              <h2 class="text-sm font-semibold text-navy">Familles</h2>
            ...
            <table mat-table [dataSource]="rows()" class="w-full">
              ...
              <ng-container matColumnDef="domain">
                ...
                <td mat-cell *matCellDef="let row" class="text-slate-600">{{ row.domainName }}</td>
              </ng-container>
...
export class SkillFamilyPage {
  readonly router = inject(Router);
  private readonly api = inject(SkillReferentialService);
  private readonly auth = inject(AuthService);
  readonly crumbs = [{ label: 'Accueil' }, { label: 'Référentiel' }, { label: 'Familles' }];
  readonly columns = ['code', 'name', 'domain', 'state', 'actions'];
  ...
  domainId = signal<string | null>(null);
  ...
  async load(): Promise<void> {
    const [domains, families] = await Promise.all([this.api.getDomains(), this.api.getFamilies()]);
    this.domainOptions.set(domains.filter((d) => d.state !== 'Archived').map((d) => ({ label: d.name, value: String(d.id) })));
    this.rows.set(families);
    if (!this.editingId()) {
  ...
  edit(row: TaxonomyItem): void {
    ...
    this.domainId.set(row.domainId ? String(row.domainId) : null);
  }
  ...
  private async refreshSuggestedCode(): Promise<void> {
    ...
    try {
      const next = await this.api.suggestCode('family');
  ...
  async save(): Promise<void> {
    ...
    try {
      ...
      if (this.editingId()) await this.api.updateFamily(this.editingId()!, payload);
      else await this.api.createFamily(payload);
      this.resetForm();
      await this.load();
    } catch (err: any) {
  ...
  async archive(row: TaxonomyItem): Promise<void> {
    try {
      await this.api.archiveFamily(row.id);
      await this.load();
    } catch (err: any) {

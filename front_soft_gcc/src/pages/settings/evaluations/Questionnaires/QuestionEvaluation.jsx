...
function QuestionEvaluation() {
    ...
    const fetchPositions = async () => {
        try {
            const response = await axios.get(urlApi('/Evaluation/postes'));
    ...
    const fetchCompetenceLines = async () => {
        try {
            ...
            const response = await axios.get(urlApi('/Evaluation/competence-lines'));
            ...
            try {
                const altResponse = await axios.get(urlApi('/CompetenceLine'));
    ...
    const fetchResponseTypes = async () => {
        try {
            // Essayer d'abord l'endpoint standard
            const response = await axios.get(urlApi('/Evaluation/response-types'));
            ...
            // Essayer un autre format d'URL
            try {
                const altResponse = await axios.get(urlApi('/ResponseType'));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (currentQuestion) {
                ...
                const response = await axios.put(urlApi(`/Evaluation/questions/${questionId}`), requestData);
                console.log("Réponse de mise à jour:", response);

                // Fermer la modale après mise à jour
                setShowEditModal(false);
            } else {
                const response = await axios.post(urlApi('/Evaluation/questions'), requestData);
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(urlApi(`/Evaluation/questions/${id}`));
            fetchAllQuestions();
        ...
        if (window.confirm(`Êtes-vous sûr de vouloir supprimer les ${selectedItems.length} questions sélectionnées ?`)) {
            try {
                ...
                await Promise.all(selectedItems.map(id => 
                    axios.delete(urlApi(`/Evaluation/questions/${id}`))
                ));

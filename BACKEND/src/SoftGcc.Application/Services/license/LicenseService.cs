...
namespace SoftGcc.Application.Services.license
{
    ...
    /// Enregistré comme Scoped car il dépend de IApplicationDbContext (Transient)
    ...
    public class LicenseService : ILicenseService
    {
        ...
        private readonly IRsaPublicKeyProvider _rsaProvider;
        ...
        public LicenseService(
            ...
            IRsaPublicKeyProvider rsaProvider,
            IMemoryCache cache)
        {
            _context = context;
        ...
        public async Task<LicenseValidationResult> Activate(LicenseActivateDto dto)
        {
            ...
            var existingLicense = await _context.Set<SoftGcc.Domain.Entities.license.License>()
            ...
            var currentLicense = await _context.Set<SoftGcc.Domain.Entities.license.License>()
                .FirstOrDefaultAsync();

            SoftGcc.Domain.Entities.license.License license;

            if (currentLicense != null && currentLicense.LicenseId != result.LicenseId!.Value)
            {
                _context.Set<SoftGcc.Domain.Entities.license.License>().Remove(currentLicense);
                license = new SoftGcc.Domain.Entities.license.License();
                _context.Set<SoftGcc.Domain.Entities.license.License>().Add(license);
            }
        ...
        public async Task<LicenseValidationResult> GetStatus()
        {
            ...
            var license = await _context.Set<SoftGcc.Domain.Entities.license.License>()
                .OrderByDescending(l => l.LastValidatedAt)
                .FirstOrDefaultAsync();
